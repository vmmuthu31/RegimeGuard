...
import { SiSolana, SiDogecoin, SiXrp, SiCardano, SiBinance, SiLitecoin } from "react-icons/si";
            ...
            >
                ...
                <div>
                    <div className="flex items-center gap-2">
                        <h1 className="text-sm font-bold text-white group-hover:text-emerald-400 transition-colors tracking-tight uppercase">{currentSymbol}</h1>
                        <FaChevronDown className={`w-2 h-2 text-emerald-500/50 transition-transform duration-300 ${isOpen ? "rotate-180" : ""}`} />
                    </div>
                </div>
            </div>
                    ...
                    >
                        <div className="p-2 flex flex-col gap-1">
                            ...
                            {TRADING_PAIRS.map((pair) => {
                                ...
                                const iconKey = pair.replace("cmt_", "").replace("usdt", "").toUpperCase();
                                ...
                                return (
                                    ...
                                    >
                                        <div className="flex items-center gap-3">
                                            <div className={`p-1.5 rounded-lg bg-zinc-950 border border-white/5 shadow-inner transition-colors ${isActive ? "text-emerald-400" : "text-zinc-500 group-hover/item:text-emerald-400"}`}>
                                                {IconMap[iconKey] || <FaBitcoin className="w-3.5 h-3.5" />}
                                            </div>
                                            <span className={`text-xs font-bold transition-colors ${isActive ? "text-white" : "text-zinc-400 group-hover/item:text-white"}`}>
                                                {cleanName}
                                            </span>
                                        </div>
                                        {isActive && (
